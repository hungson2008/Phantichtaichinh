...
# --- Hàm Xử lý Gửi Tin Nhắn Chatbot (MỚI) ---
def handle_chat_input(prompt, api_key, df_context):
    """Gửi prompt người dùng kèm theo dữ liệu phân tích đến Gemini Chatbot."""

    if "chat_service" not in st.session_state or df_context is not st.session_state.get("current_df_context"):
        ...
        try:
            ...
            st.session_state.chat_messages = [{"role": "model", "text": "Chào bạn! Tôi là Trợ lý phân tích tài chính AI. Dữ liệu của bạn đã được tải lên, bạn muốn hỏi gì về báo cáo này?"}]
        except Exception as e:
...
# Khởi tạo state cho Chatbox
if "chat_messages" not in st.session_state:
    st.session_state.chat_messages = []

# Khởi tạo biến lưu trữ DataFrame đã xử lý
df_processed = None 
...
        if df_processed is not None:
            # ... (Phần hiển thị Chức năng 2, 3, 4 giữ nguyên) ...
...
# ----------------------------------------------------------------------
# --- BỔ SUNG KHUNG CHATBOT (CHỨC NĂNG 6) ---
# ----------------------------------------------------------------------
...
if uploaded_file is not None and df_processed is not None:
    ...
    # Chuẩn bị Context cho Chatbot (Dữ liệu đã phân tích)
    # Lấy toàn bộ DataFrame đã xử lý dưới dạng Markdown để truyền ngữ cảnh
    chat_context = df_processed.to_markdown(index=False)
    api_key = st.secrets.get("GEMINI_API_KEY")
    
    if not api_key:
        st.error("Lỗi Chatbot: Không thể khởi tạo do thiếu Khóa API Gemini.")
    else:
        # Hiển thị lịch sử chat
        for message in st.session_state.chat_messages:
            with st.chat_message(message["role"]):
                # Sử dụng st.markdown để hỗ trợ hiển thị code và định dạng
                st.markdown(message["content"])

        # Khung nhập liệu chat
        user_prompt = st.chat_input("Ví dụ: 'Giải thích tốc độ tăng trưởng âm của Tài sản ngắn hạn?'")
